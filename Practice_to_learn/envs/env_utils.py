...
def load_info(env_name):
    ...
    if str(env_name) == '13':  # str() coverts a specified value to a string
        ...
        with open("./envs/dss_123/IEEE123Loads.DSS", 'r') as dssfile:
            dss_str = dssfile.readlines()  # return each line in the file as a list item

        load_base = OrderedDict()
        for s in dss_str:
            if 'New Load.' in s:
                idx = s.index("New Load.") + len('New Load.')  # index("New Load.") = 0 & len('New Load.') = 9
                name = []
                for c in s[idx:]:  # contain all sentences after New Load.
                    if c == ' ':
                        break
                    else:
                        name.append(c)  # gather all alphabets for the name of load before reaching to blanket
